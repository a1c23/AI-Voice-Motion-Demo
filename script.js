...
function animateVoice() {
  ...
  if (isAISpeaking && window.chatGPTVoice && window.chatGPTVoice.getAIAnalyser) {
    currentAnalyser = window.chatGPTVoice.getAIAnalyser();
  } else if (isUserSpeaking && window.chatGPTVoice && window.chatGPTVoice.getUserAnalyser) {
    currentAnalyser = window.chatGPTVoice.getUserAnalyser();
  }
...
function toggleVoice() {
  ...
  if (!isVoiceActive) {
    ...
    // NOTE: Don't call initAudio() here - chatGPTVoice.start() handles its own mic access
    ...
    window.chatGPTVoice.start();
  } else {
    // Stop voice conversation
    window.chatGPTVoice.stop();
    isVoiceActive = false;
...
function setAPIKey() {
  ...
  // Initialize ChatGPT voice
  window.chatGPTVoice.init(apiKey);
...
function clearAPIKey() {
  if (confirm('Are you sure you want to clear your API key?')) {
    ...
    window.chatGPTVoice.apiKey = null;
...
function loadAPIKey() {
  ...
  if (savedKey) {
    window.chatGPTVoice.init(savedKey);
...
// Expose updateStatusDisplay globally for chatgpt-voice.js
...
window.addEventListener('DOMContentLoaded', () => {
  ...
  // Set up ChatGPT voice callbacks
  window.chatGPTVoice.onStateChange = (state) => {
    setAnimationState(state);
  };

  window.chatGPTVoice.onError = (error) => {
